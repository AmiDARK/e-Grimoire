...
globalDatasBuild:
    move.l     globalDatas(a5),d0                 ; d0 = pointer to globalDatas
    tst.l      d0                                 ; d0 <> 0 ?
...
.bgdNullIsOk:
    Move.l     #glblSize,d1                       ; D1 = globalDatas buffer size
    tst.l      d1                                 ; Are some variables defined ? (d1<>0 ?)
    ...
    move.l     fvbPos(a5),d0                      ; D0 = Current position inside the fullVariablesBuffer
    tst.l      d0
    bne.s      .FullBufferIsOk
    CastErrorID FullVariableBufferNotSet
.FullBufferIsOk:
    sub.l      #glblSize,d0                       ; push d0 upper in the buffer (buffer is used from end to start)
...
globalDataSizeDefinedIsOk:
    add.l       d1,d0
    move.l      d0,fvbPos(a5)                          ; Removes GlobalDatas from fullVarBuffer by updating fvbPos pointer.
