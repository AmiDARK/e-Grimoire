...
;	Include "exec/exec_lib.i"
	...
	Include	"AmigaGS Dev:System/Libs/Graphics/Lib_Icons/Dos.i"
...
idString:      dc.b 'agaicons.library Ver1.1 '
               dc.b '680x0 Blitter Version '
			   dc.b '(30 August 00) ',13,10,0
...
__ReserveIcons:
	...
	Move.l	#$10002,d1	; D1=Memory : CHIP for BLITTER .
	Move.l	$4,a6
	Jsr		_LVOAllocMem(a6)
	Lea.l	_IcBase,a0
	Move.l	d0,(a0)
...
gi:
	...
	Rts
; * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
; Routine de paste icon avec le blitter. D0=Icon D1=Xpos D2=YPos.
__PasteIcon:
	Lea.l	ICC,a0
	Movem.l	d0-d2,(a0)	; X,Y,Icon
;
	LibCall	Screens,CurrentBase
	Tst.l	a0
	Beq.w	gi2
...
;	Move.w	(a1,76),d3
;	Sub.w	#16,d3
;	Cmp.l	d3,d1		; X>=ScreenXSize
;	bge.w	_pj
;	Tst.l	d2		; Y<0
;	Blt.w	_pj
;	Move.w	(a1,78),d3
;	Sub.w	#16,d3
;	Cmp.l	d3,d2		; Y>=ScreenYSize
;	Bge.w	_pj
;; Verifications...
;	Cmp.l	#1,d0
;	Blt.w	_pj		; Icon<1.
...
;	Bne.b	_ppw3
	Lea.l	_IcBase,a5
	Move.l	(a5),a0
	Add.l	#8,a0		; Cause : "F.C1"
	Mulu.l	#260,d0		; D0=D0*260 (260=1 icon length.) (2*16*8).
	Add.l	d0,a0		; A0=ADRESSE ICONE A DESSINER !!!!!!
	Sub.l	d3,d3
	Move.w	(a1,76),d3
	Lsr.w	#3,d3
	Mulu.l	d3,d2		; Y=Y*XSize (XSize=XScreenSize/8).
	Lsr.l	#3,d1
	Add.l	d1,d2		; D2=Rajout aux BLTDPTH/L.
; Lecture de la base du 1er bit plan.
	Move.l	a1,a5
	Move.l	(a5),a1
	Move.l	(a5,4),d5
	Sub.l	(a5),d5		; d5=1 Bitplane Depth
	add.l	d2,a1		; A1=Adresse 1ere cible; +d5=next.
	Move.w	(a5,76),d4
	Move.w	(a5,80),d6	; How many bpls
	Divu.w	#8,d4
	Sub.w	#2,d4
; Blitter occupe ?
_pj0:
...
; Mise en place de la source a lire.
	Move.l	a0,BLTAPTH
	Add.l	#32,a0		; A0 en position de next plan pour icone.
; Modulo de la source=0.
	Move.w	#$0,BLTAMOD
; Selection de la zone cible.
	Move.l	a1,BLTDPTH
	Add.l	d5,a1	; A1 en position de next screen PLANE.
; Selection du modulo de la cible
	Move.w	d4,BLTDMOD	; (XScreenSize/8)-2
; Selection du type d'operation logique pour obtenir le resultat graphique.
	Move.w	#%0000100100010000,BLTCON0	; USEA/D & LF4 Actives.
...
; Fichier au format Icones.
	...
	Move.l	#$10002,d1	; D1=Requirements.
	Move.l	$4,a6
	Jsr		_LVOAllocMem(a6)
