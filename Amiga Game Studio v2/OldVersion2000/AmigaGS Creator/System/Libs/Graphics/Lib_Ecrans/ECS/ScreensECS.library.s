...
;	Include "exec/exec_lib.i"
	...
	Include	"AmigaGS:Includes/_Chips.s"
...
LibName:       dc.b 'screensecs.library',0
idString:      dc.b 'screens.library 1.1 ECS Version '
			   dc.b '(03 Mai 2000)',13,10,0
...
;
; Verification des donnees VALIDES ???
	Exg.l	d0,d3
	Exg.l	d1,d2
	Cmp.l	#7,d3
	Bgt.w	L4VALS
...
; Verification si l'ecran existe deja.
	Lea		SCRNOPEN,a0
	Add.l	d3,a0
	Move.b	(a0),d4
	Cmp.b	#$ff,d4
...
; Sauvegarde des registres de depart.
	Lea		L4SCREEN(pc),a0
	Move.l	d3,(a0)
	Lea		L4XSIZE(pc),a0
	Move.l	d2,(a0)
...
; Calcul de la longueur de 1 bit plane.
	Lsr.l	#3,d2
	Mulu.l	d2,d1		; D1=Longueur de 1 bit plane.
	Lea		L4BPLENGTH(pc),a0
	Move.l	d1,(a0)
	Mulu.l	d1,d0		; D0=Longueur de x bits planes.
	Lea		L4LENGTH(pc),a0
	Move.l	d0,(a0)

; Reservation de la memoire alouee pour l'ecran.
	Add.l	#1024,d0	; D0+1024 pour la screen base,palette,etc...
	Move.l	#$10002,d1
	ExecC	AllocMem
	Cmp.l	$0,d0
	Beq.w	L4NOMEM
...
; Mise en place de l'adresse de depart de chaque bits plans dans la BASE.
	Move.l	d0,a0		; A0=Screen base adress
	Lea		L4DEPTH(pc),a1
	Move.l	(a1),d0		; D0=nombre de bits plans.
	Move.l	a0,d1
	Add.l	#1024,d1	; D1=1st bit planes base.
	Lea		L4BPLENGTH(pc),a1
	Move.l	(a1),d2		; D2=1bpl length.
L4BCL1
	Move.l	d1,(a0)+
	Add.l	d2,d1
	Sub.l	#1,d0
...
; Mise en place des autres donnees dans la base.
	Lea		L4BASE(pc),a2
	Move.l	(a2),a0
	Lea		L4LENGTH(pc),a1
	Move.l	(a1),d0
...
; Mise en place dans la zone de donnees de la librairie.
	Lea		SCRNOPEN,a0
	Lea		L4SCREEN(pc),a1
	Move.l	(a1),d0
	Add.l	d0,a0
	Move.b	#$ff,(a0)	; Screen actif.
	Lea		SCRNBASE,a0
	Lsl.l	#2,d0
	Add.l	d0,a0
	Lea		L4BASE(pc),a1
	Move.l	(a1),(a0)	; Screen base sauvee.

; Activation en tant que ECRAN COURANT !!!
	Lea		SCRNCURRENT,a0
	Lea		L4BASE(pc),a1
	Move.l	(a1),(a0)
...
L4OPEN
	...
	Rts
;
L4SCREEN	Dc.l	0	; Screen 0-7
L4XSIZE		Dc.l	0	; Screen x size.
L4YSIZE		Dc.l	0	; Screen y size.
L4DEPTH		Dc.l	0	; how many bits planes ?
L4LENGTH	Dc.l	0	; total length -1024 (screen bases)
L4BPLENGTH	Dc.l	0	; 1 bit plane length.
L4BASE		Dc.l	0	; Screen base.
;
;
;
; **************************************************************************
