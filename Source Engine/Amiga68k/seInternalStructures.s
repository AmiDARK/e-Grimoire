...
AllocSys:
	tst.l	SysStructBackup(pc) 			; Verify is System Structure was already allocated or not
	bne.s 	.asEnd							; If != 0 -> .asEnd (no new allocation)
	Move.l	#SysStructureLen,d0 			; D0 = System Structure Bytes Length
	Move.l 	#Public|Clear,d1 				; D1 = Fast Mem if availables + Clear Memory
	ExeCall AllocMem 						; AllocMem -> A0
	Move.l	a0,SysStructBackup(pc) 			; Save System Structure buffer pointer.
.asEnd:
	rts

loadSys:
	Move.l 	SysStructBackup(pc),a5 			; A5 = Pointer to Internal System Structure
	rts

; *********************************************
; This method release memory used for the Source Engine internal structure
FreeSys:
	Move.l	SysStructBackup(pc),a0 			; A0 = Pointer to the Internal System Structure
	Beq.s 	.fsEnd 							; A0 = 0 -> .fsEnd
	ExeCall	FreeMem 						; Release memory used by Internal System Structure
.fsEnd:
	clr.l	SysStructBackup(pc) 			; Clear memory to be sure it will no more be used
	rts
