...
proc\<$inProcName>\1_Label:
    loadLocalDatas a4
    move.w            #TypeStr,proc\<$inProcName>\1+4(a4) ; Setup the Local variable as Integer variable
  ; **** 1.2 If a 2nd argument is set, we try to detect it and use it, otherwise we let the variable to its default value
    IFEQ NARG-2                                          ; If VALUE is set, we must affect it to the variable itself
      lea             proc\<$inProcName>\1_Data(pc),a3
      move.l          a3,d7
      move.l          d7,proc\<$inProcName>\1(a4)        ; Setup the Local variable Integer value
