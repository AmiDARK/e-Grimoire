...
LibName:       dc.b 'memory680x0.library',0
idString:      dc.b 'memory.library Ver1.0 '
               dc.b '68000/10/20/30 Version '
...
; Out	:   ...
Memory_Clear:
	Btst	#0,d0		; IF d0 IS impair THEN _BytesClear
	Bne.b	_BytesClear
	Btst	#1,d0		; IF d0 IS bytes0&1=0 THEN _LongWordClear
	Beq.b	_LongWordClear
;
_WordClear:
	Subq.l	#1,d0		; For Bpl.s use instead of 'tst.b / beq.b'
wc1:
	Clr.w	(a0)+
	Subq.l	#2,d0
	Bpl.s	wc1
...
; Out	:   ...
Memory_Copy:
	Btst	#0,d0			; IF d0 IS impair THEN _BytesCopy
	Bne.b	_BytesCopy
	Btst	#1,d0			; IF d0 IS bytes0&1=0 THEN _LongWordCopy
	Beq.b	_LongWordCopy
;
_WordCopy:
	Subq.l	#1,d0		; For Bpl.s use instead of 'tst.b / beq.b'
